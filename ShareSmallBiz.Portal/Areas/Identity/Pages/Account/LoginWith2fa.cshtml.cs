...
namespace ShareSmallBiz.Portal.Areas.Identity.Pages.Account
{
    public class LoginWith2faModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("User with ID '{CreatedID}' logged in with 2fa.", user.Id);
                return LocalRedirect(returnUrl);
            }
            else if (result.IsLockedOut)
            {
                _logger.LogWarning("User with ID '{CreatedID}' account locked out.", user.Id);
                return RedirectToPage("./Lockout");
            }
            else
            {
                _logger.LogWarning("Invalid authenticator code entered for user with ID '{CreatedID}'.", user.Id);
