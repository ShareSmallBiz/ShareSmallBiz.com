...
public class MenuProvider(ShareSmallBizUserContext webDomainContext)
    : IMenuProvider, IDisposable, IMenuService
{
    ...
    private Menu Create(Models.MenuModel menu)
    {
        ...
        var parentMenu = webDomainContext.Menus.Where(w => w.Id == menu.ParentId).FirstOrDefault();
        if (parentMenu != null)
        {
    ...
    {
        int displayOrder = 10;
        foreach (Menu item in webDomainContext.Menus.OrderBy(o => o.DisplayOrder))
        {
    ...
    {
        var deleteItem = webDomainContext.Menus.Where(w => w.Id == Id).FirstOrDefault();
        if (deleteItem != null)
        {
            webDomainContext.Menus.Remove(deleteItem);
            webDomainContext.SaveChanges();
            return true;
        }
        return false;
    ...
    {
        var menuList = webDomainContext.Menus.Include(i => i.Domain).ToList();
    ...
    {
        var returnMenu = Create(webDomainContext.Menus.Where(w => w.Id == Id)
            .Include(i => i.Parent)
            .Include(i => i.Domain)
            .FirstOrDefault());
        returnMenu ??= new Models.MenuModel();
    ...
    {
        return Create(webDomainContext.Menus.OrderBy(o => o.DisplayOrder)
            .Include(i => i.Parent)
            .Include(i => i.Domain)
    ...
    {
        return Create(webDomainContext.Menus.Where(w => w.Domain.Id == DomainId)
            .Include(i => i.Parent)
            .Include(i => i.Domain)
            .OrderBy(o => o.DisplayOrder)
            .ToList());
    ...
    public Models.MenuModel Save(Models.MenuModel saveItem)
    {
        ...
        if (saveItem.Id == 0)
        {
            ...
            try
            {
                webDomainContext.Menus.Add(saveMenu);
                webDomainContext.SaveChanges();
        ...
        else
        {
            try
            {
                var dbMenu = webDomainContext.Menus.Where(w => w.Id == saveItem.Id).FirstOrDefault();
                var parentMenu = webDomainContext.Menus.Where(w => w.Id == saveItem.ParentId).FirstOrDefault();
    ...
    {
        var dbMenu = webDomainContext.Menus.Where(w => w.Id == Id).FirstOrDefault();
        if (dbMenu != null)
        {
            try
            {
                var childMenu = webDomainContext.Menus.Where(w => w.Parent.Id == Id);
                foreach (var child in childMenu)
                {
                    child.Parent = null;
                }
                webDomainContext.Menus.Remove(dbMenu);
    ...
    {
        var menu = await webDomainContext.Menus.FindAsync(menuId);
    ...
    {
        var menu = await webDomainContext.Menus.Include(m => m.Keywords).FirstOrDefaultAsync(m => m.Id == menuId);
